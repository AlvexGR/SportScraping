...
{
    public class NextBetPlayerOverUnderJob : ScrapeJob
    {
        public NextBetPlayerOverUnderJob(WebPortalHelper webPortalHelper, ScrapeHelper scrapeHelper)
            : base(webPortalHelper, scrapeHelper)
        {
            Logger = Helper
                .GetLoggerConfig($@"{NBAScrapingService.BaseLoggerPath}\Scrape\PlayerOverUnder\nextbet-.txt")
                .CreateLogger();
            ProviderType = typeof(NextBetPlayerOverUnder);
        }
    }
}
