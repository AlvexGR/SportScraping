...
{
    public class BetVictorPlayerOverUnderJob : ScrapeJob
    {
        public BetVictorPlayerOverUnderJob(WebPortalHelper webPortalHelper, ScrapeHelper scrapeHelper)
            : base(webPortalHelper, scrapeHelper)
        {
            Logger = Helper
                .GetLoggerConfig($@"{NBAScrapingService.BaseLoggerPath}\Scrape\PlayerOverUnder\betvictor-.txt")
                .CreateLogger();
            ProviderType = typeof(BetVictorPlayerOverUnder);
        }
    }
}
